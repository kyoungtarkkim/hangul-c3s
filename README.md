...
- 가변 받침이라고 하면... "그걸 어떻게 하나하나씩 상황에 맞게 다 외우고 쳐? 더 헷갈리는 것 아냐?" 하고 처음에 의문을 가지실 수 있습니다. 하지만, 제가 오랜 시간 동안 신세벌식으로 타자 연습을 해 본 결과 초성, 중성, 종성 각각을 위치별로 외워서 상황에 따라서 그것들을 조합을 하여 타자를 친다기보다는 "초성 + 중종성"을 몸이 기억하는 듯 합니다. 즉, 중성과 종성을 결합하는 방식 자체를 뇌에서 기억하기 때문에 중성과 종성이 같이 이어져서 결합하는 방식에 익숙해지게 되면 더 이상 상황에 따라 음운들을 판단하면서 타자하는 것이 아니라, 몸이 알아서 움직이게 된다는 것이죠... 따라서, 굳이 가변 받침을 도입하여 손가락을 편하게 만드는 이 방식은 실제로 정확성과 속도면에서 효과가 좋습니다: 상황에 따라 왼손 손가락들을 다르게 구현하는 하는 참신세벌식D의 경우 손가락이 계속 꼬이게 되는 경험을 많이 하게 되는데 이러한 상황보다 훨씬 낳다고 판단하여 가변 받침을 도입하게 되었습니다!!! 
